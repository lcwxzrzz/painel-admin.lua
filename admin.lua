...
local function executeCrash(targetPlayer)
    ...
    -- Tenta sobrecarregar o cliente do alvo com muitas partes e efeitos
    for i = 1, 500 do -- Cria 500 partes pequenas e transparentes
    ...
    -- Tenta criar muitos efeitos visuais (ex: partículas)
    local particleEmitter = Instance.new("ParticleEmitter")
    particleEmitter.Rate = 1000 -- Muitas partículas
    ...
    -- Tenta disparar muitos sons (pode ser irritante, mas contribui para a sobrecarga)
    for i = 1, 10 do
...
-- Placeholder para funcionalidades de casas (client-side)
local function unbanAllHouses()
    Say(";unbanallhouses") -- Assume um comando server-side para isso
    warn("Comando para desbanir de todas as casas enviado.")
end

local function autoBanHouse(player)
    -- Isso seria mais complexo, envolvendo detecção de entrada em uma área e banimento.
    -- Por enquanto, um placeholder para um comando server-side.
    if player then
        Say(";autobanhouse " .. player.Name) -- Assume um comando server-side
...
local function antiBanHouseToggle(enabled)
    if enabled then
        -- Implementação de anti-ban-casa (client-side)
        -- Isso envolveria hooks em RemoteEvents/Functions que o servidor usa para banir de casas.
        -- Exemplo genérico (pode precisar de ajustes específicos do jogo):
        local oldFireServer = remoteEvent.FireServer
        remoteEvent.FireServer = function(self, eventName, ...)
            if eventName == "BanFromHouse" then -- Exemplo de nome de evento de ban de casa
...
SectionActions:Button({
    ...
    SectionActions:Button({
        Title = ";aura",
        Desc = "Cria uma aura de cadeiras giratórias ao seu redor.",
...
SectionVisTarget:Button({
    ...
    SectionHouses:Button({
        ...
        Desc = "Tenta banir automaticamente jogadores que entram em uma casa (requer script server-side e configuração).",
